<br><br>
<!--forma zamovyty poslugu -->
<div class="container" style="text-align:center;">
    <h3>Успіх мого бізнесу вимірюється успішною допомогою Вашому бізнесу.
        Давайте розпочнемо співпрацю, що сприятиме Вашому розвитку!</h3><br><br>
</div>
<div class="container">
    <div class="row">
         <div class="col-sm-12 col-lg-4 offset-lg-4">

 <!-- start modal from gpt -->

<!-- Кнопка для відкриття модального вікна -->
<button type="button" class="btn zoryana_btn" data-bs-toggle="modal" data-bs-target="#exampleModal">
  <span>Замовити послугу</span>
</button>

<!-- Модальне вікно -->
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title" id="exampleModalLabel"  >Замовити послугу</h1>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <!-- Вставка шорткоду Contact Form 7 -->
        <?php echo do_shortcode('[contact-form-7 id="e713535" title="ЗАМОВИТИ ПОСЛУГУ"]'); ?>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Закрити</button>
      </div>
    </div>
  </div>
</div>




       <!-- end modal code -->
            	

        <!-- modal form -->
<div id="modal" class="modal">
    <div class="modal-content">
        <div class="modal-header">
            <!-- Додатковий контент -->
        </div>
        <div class="container">
            <div class="row">
                <div class="col">
                  
         

                </div>
            </div>
        </div>
    </div>
</div>

          
        </div>
     </div>
</div>
<!-- end forma zamovyty poslugu -->


<div class="container" style="text-align:center;">
    <h3>Працюємо для розвитку Вашого бізнесу!</h3> <br><br><br>
</div>
<footer>
  <div class="container" style="padding-top: 30px;">
     <div class="row">
        <a name="kontaktyblock"></a>
        <div class="col-lg-10 col-sm-6 col-xs-6">
             <h3 style="margin-bottom: -10px;">Контакти </h3><br>
             <p>м. Київ<br>
                 <a class="telefon" style="color: white; font-family: Inter;" href="tel:+38068 920 22 01">+38068 920 22 01</a> <br>
                    zoryana.projects@gmail.com
             </p>
        </div>
        <div class="col-lg-2 col-sm-6 col-xs-6">
            <a href="https://www.linkedin.com/in/zoriana-filipovs-70b57b279/">  
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                    <path d="M0 1.43267C0 0.641641 0.657515 0 1.46864 0H18.5314C19.3425 0 20 0.641644 20 1.43268V18.5673C20 19.3586 19.3425 20 18.5314 20L1.46864 20C0.657515 20 0 19.3586 0 18.5673V1.43267ZM6.17922 16.7421V7.71095H3.17745V16.7421H6.17922ZM4.67832 6.4779C5.7251 6.4779 6.37664 5.78441 6.37664 4.91775C6.35713 4.03159 5.7251 3.35734 4.69818 3.35734C3.67136 3.35734 3 4.03159 3 4.91775C3 5.78441 3.65139 6.4779 4.65876 6.4779H4.67832ZM10.8142 16.7421V11.6987C10.8142 11.4288 10.8338 11.1592 10.913 10.9662C11.13 10.4269 11.6239 9.86838 12.4532 9.86838C13.5394 9.86838 13.9739 10.6966 13.9739 11.9106V16.7421H16.9754V11.5638C16.9754 8.7898 15.4945 7.49903 13.5196 7.49903C11.9272 7.49903 11.2133 8.37421 10.8143 8.98959V9.0207H10.7943C10.8009 9.0104 10.8076 9.00003 10.8143 8.98959V7.71095H7.81258C7.85197 8.55837 7.81258 16.7421 7.81258 16.7421H10.8142Z" fill="white"/>
                </svg>
             </a>    
             <a href="https://www.facebook.com/people/Filipovs-Consulting/61551231173530/">
                  <svg xmlns="http://www.w3.org/2000/svg" width="21" height="20" viewBox="0 0 21 20" fill="none">
                     <path d="M20.9997 10.0605C20.9997 4.50371 16.5223 -0.000976562 10.9991 -0.000976562C5.47595 -0.000976562 0.998535 4.50371 0.998535 10.0605C0.998535 15.0825 4.6556 19.245 9.43653 19.9998V12.9689H6.89732V10.0605H9.43653V7.84386C9.43653 5.32219 10.9295 3.9293 13.2139 3.9293C14.308 3.9293 15.4525 4.12581 15.4525 4.12581V6.60189H14.1915C12.9491 6.60189 12.5617 7.37747 12.5617 8.17315V10.0605H15.3353L14.8919 12.9689H12.5617V19.9998C17.3426 19.245 20.9997 15.0825 20.9997 10.0605Z" fill="white"/>
                  </svg>
             </a>    
              <a href="https://www.instagram.com/filipovs_consulting/">
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                   <path d="M10.0009 0C7.28508 0 6.94424 0.0118753 5.87756 0.0604175C4.81297 0.109168 4.08629 0.277711 3.45045 0.525006C2.79274 0.780425 2.23481 1.12209 1.67898 1.67814C1.12272 2.23398 0.78105 2.7919 0.524797 3.44941C0.276878 4.08546 0.108126 4.81234 0.0602089 5.87652C0.0125 6.9432 0 7.28424 0 10.0001C0 12.716 0.0120837 13.0558 0.0604175 14.1224C0.109376 15.187 0.27792 15.9137 0.525006 16.5495C0.780633 17.2073 1.1223 17.7652 1.67835 18.321C2.23398 18.8773 2.7919 19.2198 3.4492 19.4752C4.08546 19.7225 4.81234 19.891 5.87673 19.9398C6.94341 19.9883 7.28403 20.0002 9.99969 20.0002C12.7158 20.0002 13.0556 19.9883 14.1222 19.9398C15.1868 19.891 15.9143 19.7225 16.5506 19.4752C17.2081 19.2198 17.7652 18.8773 18.3208 18.321C18.8771 17.7652 19.2187 17.2073 19.475 16.5498C19.7208 15.9137 19.8896 15.1868 19.9396 14.1226C19.9875 13.056 20 12.716 20 10.0001C20 7.28424 19.9875 6.9434 19.9396 5.87673C19.8896 4.81213 19.7208 4.08546 19.475 3.44962C19.2187 2.7919 18.8771 2.23398 18.3208 1.67814C17.7646 1.12189 17.2083 0.780216 16.55 0.525006C15.9125 0.277711 15.1854 0.109168 14.1208 0.0604175C13.0541 0.0118753 12.7145 0 9.99781 0H10.0009ZM9.10385 1.8021C9.3701 1.80169 9.66718 1.8021 10.0009 1.8021C12.671 1.8021 12.9874 1.81169 14.0418 1.8596C15.0168 1.90419 15.546 2.0671 15.8985 2.20398C16.3652 2.38523 16.6979 2.6019 17.0477 2.95191C17.3977 3.30191 17.6143 3.63525 17.796 4.10192C17.9329 4.454 18.096 4.98318 18.1404 5.95819C18.1883 7.01236 18.1987 7.32903 18.1987 9.99781C18.1987 12.6666 18.1883 12.9833 18.1404 14.0374C18.0958 15.0124 17.9329 15.5416 17.796 15.8937C17.6148 16.3604 17.3977 16.6927 17.0477 17.0425C16.6977 17.3925 16.3654 17.6091 15.8985 17.7904C15.5464 17.9279 15.0168 18.0904 14.0418 18.135C12.9876 18.1829 12.671 18.1933 10.0009 18.1933C7.3307 18.1933 7.01424 18.1829 5.96006 18.135C4.98505 18.09 4.45588 17.9271 4.10317 17.7902C3.6365 17.6089 3.30316 17.3923 2.95316 17.0423C2.60315 16.6923 2.38648 16.3598 2.20481 15.8929C2.06794 15.5408 1.90481 15.0116 1.86044 14.0366C1.81252 12.9824 1.80294 12.6658 1.80294 9.99531C1.80294 7.32487 1.81252 7.00986 1.86044 5.95569C1.90502 4.98068 2.06794 4.4515 2.20481 4.099C2.38607 3.63233 2.60315 3.29899 2.95316 2.94899C3.30316 2.59898 3.6365 2.38232 4.10317 2.20065C4.45567 2.06315 4.98505 1.90064 5.96006 1.85585C6.88257 1.81419 7.24008 1.80169 9.10385 1.7996V1.8021ZM15.3389 3.46254C14.6764 3.46254 14.1389 3.99942 14.1389 4.66213C14.1389 5.32464 14.6764 5.86214 15.3389 5.86214C16.0014 5.86214 16.5389 5.32464 16.5389 4.66213C16.5389 3.99962 16.0014 3.46212 15.3389 3.46212V3.46254ZM10.0009 4.86463C7.16487 4.86463 4.86547 7.16403 4.86547 10.0001C4.86547 12.8362 7.16487 15.1345 10.0009 15.1345C12.837 15.1345 15.1356 12.8362 15.1356 10.0001C15.1356 7.16403 12.8368 4.86463 10.0007 4.86463H10.0009ZM10.0009 6.66674C11.8418 6.66674 13.3343 8.15904 13.3343 10.0001C13.3343 11.841 11.8418 13.3335 10.0009 13.3335C8.15988 13.3335 6.66757 11.841 6.66757 10.0001C6.66757 8.15904 8.15988 6.66674 10.0009 6.66674Z" fill="white"/>
               </svg>
             </a> 
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                <path d="M15.2072 11.1747C14.5522 12.5019 13.6656 13.8251 12.7572 15.0126C11.8517 16.1963 10.943 17.221 10.2596 17.9506C10.1688 18.0475 10.0821 18.1391 10 18.2251C9.91793 18.1391 9.83121 18.0475 9.7404 17.9506C9.05697 17.221 8.14835 16.1963 7.24281 15.0126C6.33443 13.8251 5.44781 12.5019 4.79279 11.1747C4.13101 9.83384 3.75 8.57738 3.75 7.5C3.75 4.04822 6.54822 1.25 10 1.25C13.4518 1.25 16.25 4.04822 16.25 7.5C16.25 8.57738 15.869 9.83384 15.2072 11.1747ZM10 20C10 20 17.5 12.8921 17.5 7.5C17.5 3.35786 14.1421 0 10 0C5.85786 0 2.5 3.35786 2.5 7.5C2.5 12.8921 10 20 10 20Z" fill="white"/>
                <path d="M10 10C8.61929 10 7.5 8.88071 7.5 7.5C7.5 6.11929 8.61929 5 10 5C11.3807 5 12.5 6.11929 12.5 7.5C12.5 8.88071 11.3807 10 10 10ZM10 11.25C12.0711 11.25 13.75 9.57107 13.75 7.5C13.75 5.42893 12.0711 3.75 10 3.75C7.92893 3.75 6.25 5.42893 6.25 7.5C6.25 9.57107 7.92893 11.25 10 11.25Z" fill="white"/>
             </svg>
            <a href="https://www.tiktok.com/@zorianafilipovs?_t=8g3U00QoN2T&_r=1&fbclid=IwAR0eOfw6DGWSfhI982xHMKgy5jMOmmTpVKQh3NzkFzQ3Mjp71qPiF0ghAiM">
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                     <path d="M11.25 1.27405e-05L13.7257 0C13.9045 0.893764 14.4007 2.02132 15.2688 3.13993C16.1193 4.236 17.2464 5.00001 18.75 5.00001V7.50001C16.5583 7.50001 14.9113 6.48306 13.75 5.21424V13.75C13.75 17.2018 10.9518 20 7.5 20C4.04822 20 1.25 17.2018 1.25 13.75C1.25 10.2982 4.04822 7.50001 7.5 7.50001V10C5.42893 10 3.75 11.6789 3.75 13.75C3.75 15.8211 5.42893 17.5 7.5 17.5C9.57107 17.5 11.25 15.8211 11.25 13.75V1.27405e-05Z" fill="white"/>
                 </svg>
            </a> 
        </div>
    </div>
  </div>



    </footer>
    <?php wp_footer(); ?>
</body>
</html>